...
class plgSystemBfacebooksqueeze extends JPlugin
{
	function onAfterDispatch()
	{
		...
		if (!$app->isSite() || JRequest::getString('tmpl') == 'component' || JRequest::getVar('bfacebooksqueeze_state', '', 'cookie', 'string'))
			return;
		...
		$mid = abs(intval($this->params->get('moduleid', 0)));
		
                setcookie('bfacebooksqueeze_state', true, time() + $interval, '/');
		...
		if (!empty($mid)) {
			...
			$db = JFactory::getDBO();
			$db->setQuery("SELECT * FROM `#__modules` WHERE id='$mid'");
...
window.addEvent('domready', function() { 
	setTimeout(function() {
		SqueezeBox.setOptions({size: {x: $width, y: $height}, classWindow: 'bfacebooksqueeze'});
		SqueezeBox.assignOptions();
		SqueezeBox.setContent('adopt', jQuery('#bfacebooksqueeze-$mid').detach().css('display', 'block'));
		if ($hidetimer > 0) {
			setTimeout(function() {
				SqueezeBox.close();
			}, $hidetimer);
...
EOD;
				...
				echo "<div id='bfacebooksqueeze-$mid' style='display: none;'>$render</div>";
			}
		}
...
} // class plgSystemBfacebooksqueeze